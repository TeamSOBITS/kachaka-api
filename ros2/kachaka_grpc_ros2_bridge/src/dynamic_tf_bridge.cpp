...
bool ConvertGrpcTfToRosTf(
    ...
    tf2_msgs::msg::TFMessage* msg, const std::string& frame_prefix = "", const bool& use_map = true) {
  ...
  for (const auto& transform_grpc : grpc_msg.transforms()) {
    ...
    if ((!use_map) && (transform_ros.header.frame_id == frame_prefix + "map")) continue;
...
TfStreamClient::TfStreamClient(
    std::string frame_prefix, bool use_map,
...
void TfStreamClient::ReadStream() {
  ...
  while (reader->Read(&response)) {
    ...
    ConvertGrpcTfToRosTf(response, &msg, frame_prefix_, use_map_);
